...
def test_dbt_cli(global_config: List[str], command: str) -> None:
    dbt = DbtClientV2(project_dir=TEST_PROJECT_DIR, global_config=global_config)
...
def test_no_default_asset_events_emitted(data: dict) -> None:
    ...
    asset_events = DbtCliEventV2(event={"data": data}).to_default_asset_events(manifest=manifest)
...
def test_to_default_asset_output_events() -> None:
    ...
    asset_events = list(DbtCliEventV2(event=event).to_default_asset_events(manifest=manifest))
